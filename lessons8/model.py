...
def ReturnEnterMenu(phoneBook):
    ...
    while step != 10:
        if step == 1:
            ...
            SortPhonBook(phoneBook)
            step = EnterNumMenu()
        elif step == 10:
            exit()


def SortPhonBook(phoneBook):
    view.PrintSortMenu()
    i = int(input('Введите поле по которому будем сортировать: '))-1
    phoneBook.sort(key=lambda x: (x[i]))
    PrintPhoneBook(phoneBook)